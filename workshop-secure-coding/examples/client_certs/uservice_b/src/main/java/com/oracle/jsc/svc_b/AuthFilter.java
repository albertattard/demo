...
/**
 * The following code uses just a filter to read the DN. An authentication manager could also be
 * used while using Spring security.
 * 
...
public class AuthFilter extends OncePerRequestFilter {
    ...
    /**
     * This method filters the request and gathers the DN. So it can be verified in this filter.
     * 
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (certs != null && certs.length > 0) {
            ...
            String cn = gatherSubjectMap(dn).get("CN");

            log.info("Inbound request from CN=={}", cn);

            if (! dn.contains("OU=Linux_Sales,O=Oracle,L=Redwood_Shores,ST=CA,C=US") || cn != "uService-A") {

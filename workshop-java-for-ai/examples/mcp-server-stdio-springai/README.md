...
- [STDIO and SSE MCP Servers](https://docs.spring.io/spring-ai/reference/api/mcp/mcp-stdio-sse-server-boot-starter-docs.html)
...
1. Build the application
   ...
   ```
   ./target
   ├── mcp-server-stdio-springai-1.0.0.jar
   └── mcp-server-stdio-springai-1.0.0.jar.original

   1 directory, 2 files
   ```

2. Build the MCP inspector

   There are several ways to run the MCP Inspector. Using Docker ensures that
   ...
   docker build \
     ...
     --tag 'demo:mcp-server-stdio-springai' \
     --load \
     .
   ```
...
3. Run the MCP inspector
   ...
   Run the container and provide a value for the `MCP_PROXY_AUTH_TOKEN`
   environment variable, which will be used as the authentication key when
   ...
   docker run \
     --rm \
     --detach \
     --name 'mcp-server-stdio-springai' \
     --env MCP_PROXY_AUTH_TOKEN=350ba3b5-47f3-475c-83f7-da297dd74ebb \
     --publish 6274:6274 \
     --publish 6277:6277 \
     'demo:mcp-server-stdio-springai'
   ```

   This will start the MCP Inspector in the background. Allow a few seconds for
   ...
   Set the **Transport Type** to *STDIO*, enter *java* in the **Command** field,
   and specify *-jar mcp-server-stdio-springai-1.0.0.jar* in the
   **Arguments** field.

   ![Connect](./assets/images/Connect.png)

   Then select **Connect** to launch the Java application and establish a
   connection using the MCP standard input/output (STDIO) protocol.
...
4. Stop the MCP inspector when ready

   ```shell
   docker rm --force 'mcp-server-stdio-springai' >/dev/null 2>&1 || true
   ```

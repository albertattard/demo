...
class RecipeMcpServerTest {
    ...
    private static final ServerParameters serverParameters = ServerParameters.builder("java")
            .args(new String[]{"-jar", "./target/mcp-server-stdio-springai-1.0.0.jar"})
            .build();
    private static final McpJsonMapper mcpJsonMapper = McpJsonDefaults.getMapper();
    private static final McpClientTransport transport = new StdioClientTransport(serverParameters, mcpJsonMapper);
                ...
                return json -> {
                    try {
                        return McpJsonDefaults.getMapper()
                                .readValue(json, new TypeRef<List<Recipe>>() {
                                }).stream();
